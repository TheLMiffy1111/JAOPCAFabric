...
public class CompressorRecipeSupplier implements Supplier<RebornRecipe> {
	...
	public CompressorRecipeSupplier(Identifier key, Object input, int inputCount, Object output, int outputCount, int power, int time) {
	...
	public RebornRecipe get() {
		...
		stacks.add(stack);
		return new RebornRecipe(ModRecipes.COMPRESSOR, key, ings, stacks, power, time);
	}
}
